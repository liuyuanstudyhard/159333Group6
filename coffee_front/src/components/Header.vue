    ...
    <div class="header" :style="{ margin: '0 auto', position: 'fixed', top: 0, left: 0, right: 0, zIndex: 1000 }">
        ...
        >
            ...
            <div class="checkout-summary">
                ...
                <el-button type="success" @click="checkout" size="large">Checkout • ${{ totalPrice.toFixed(2) }}</el-button>
            </div>
        </el-dialog>
...
export default {
    ...
    computed: {
        ...
        isLoggedIn() {
            ...
            return isLogin; // 如果 userId 不是 -1，表示已登录
        },
      isAdmin(){
    ...
    methods: {
        ...
        logout() {
            this.$store.commit('setUserId', -1); // 清空 Vuex 中的 userId，表示用户未登录

            const redirectPath = this.$store.state.redirectPath || '/';
                    this.$store.commit('clearRedirectPath');  // 登录成功后清除跳转路径
                    this.$router.push(redirectPath);
        },
      goToOrderManagement() {
        this.$router.push({ name: 'orderManagement' }); // 跳转到订单管理页面
      },
        showCartDialog() {
        ...
        checkout() {
            ...
            // this.callFeaturedMethod();
          const userId = this.$store.getters.getUserId; // 获取用户ID
          ...
          const orderData = {
            ...
            address: this.$store.getters.getAddress,     // 地址
            price: this.totalPrice.toFixed(2),  // 总价格
            userId: userId,           // 用户ID
